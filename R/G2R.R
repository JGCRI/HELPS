...
#' @examples
#' \dontrun{
#' reg_annual_value <- G2R(grid_annual_value = pwc.hothaps.ann, SECTOR = "MAIZ_R", rast_boundary = country_raster)
#' }


G2R <- function(grid_annual_value, SECTOR, rast_boundary){
...
smw_df <- smw %>% as.data.frame(xy = T) %>% setNames(c("x", "y", "smw"))
